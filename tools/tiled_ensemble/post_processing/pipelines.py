...
def get_stats_pipeline(config: DictConfig | ListConfig, tiler: EnsembleTiler) -> EnsemblePostProcessPipeline:
    ...
    if (
        ...
        or config.ensemble.post_processing.normalization == NormalizationStage.IMAGE
    ):
...
def get_postprocessing_pipeline(
    ...
    if config.ensemble.post_processing.normalization == NormalizationStage.IMAGE:
    ...
    if config.ensemble.metrics.threshold.stage == NormalizationStage.IMAGE:
